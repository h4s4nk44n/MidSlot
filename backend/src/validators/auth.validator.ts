...
export const registerSchema = z
  .object({
    ...
    // Demographics (required at signup)
    dateOfBirth: z.union([z.string().datetime(), z.string().date()]),
    gender: z.enum(REGISTER_GENDERS),
    // Identity (required at signup)
    nationalId: z.string().trim().min(4, "National ID is required").max(32),
  })
  .refine(
    ({ password, name, email }) => {
