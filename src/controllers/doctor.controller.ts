...
export const getDashboard = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const completedAppointments = await prisma.appointment.count({ where: { doctorId: doctor.id, status: "COMPLETED" } });
        const cancelledAppointments = await prisma.appointment.count({ where: { doctorId: doctor.id, status: "CANCELLED" } });
